...
let g:minimap_highlight_range=5
...
function RunCode()
    ...
    if file_ext=='py'
        if match(readfile(expand("%:p")),"CircuitPython")!=-1
            :w!
            :call system("cp * /run/media/tom/CIRCUITPY/")
        else
            :w!
            :term ./%
        endif
    ...
    elseif file_ext=='sh'
        ...
        :term sh ./%
    elseif file_ext=='cpp'
        if match(readfile(expand("%:p")),"picosystem")!=-1
            :w!
            :!cmake ..
            :!make
            :call system("cp *.uf2 /run/media/tom/RPI-RP2/")
        elseif match(readfile(expand("%:p")),"using namespace blit;")!=-1
            :w
